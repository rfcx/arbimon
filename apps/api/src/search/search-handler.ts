...
export const searchHandler: Handler<SearchResponse, unknown, SearchRequestQueryParams> = async (request, reply) => {
  const { type, q, isPublished, limit, offset } = request.query
  if (type === undefined) {
    throw new BioPublicError('invalid query parameter "type" cannot be empty', 400)
  }
  let status
  if ([undefined, 'false'].includes(isPublished)) {
    status = false
  } else status = true
  ...
  const result = await searchDatabase(type, q === undefined ? '' : q, status, parsedLimit, parsedOffset)
