...
export const createProject = async (projectPartial: Pick<Project, 'idArbimon' | 'idCore' | 'name' | 'projectType'>, hidden: boolean = false): Promise<{ id: number, slug: string }> => {
  ...
  const projectType = projectPartial.projectType ?? 'free'
  ...
  const project: Omit<Project, 'id'> = {
    ...
    projectType,
    isLocked: false,
    status: hidden ? 'hidden' : (projectType === 'free' ? 'published' : 'unlisted'),
