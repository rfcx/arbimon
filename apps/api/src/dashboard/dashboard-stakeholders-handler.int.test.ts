...
import { createProject } from '@/projects/dao/project-create-dao'
import { create as createMember } from '@/projects/dao/project-member-dao'
...
describe(`GET ${dashboardStakeholdersRoute}`, async () => {
  beforeAll(async () => {
    const project = await createProject({ idArbimon: 1928, idCore: 'xi2334osd9', name: 'Grey-blue humpback whales' })
    const locationProjectId = project.id
    await createMember({ locationProjectId, userId: currentUserId, role: 'owner', ranking: RANKING_PRIMARY })
