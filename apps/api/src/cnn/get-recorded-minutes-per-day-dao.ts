...
export const getRecordedMinutesPerDay = async (projectId: string): Promise<Record<string, number>> => {
  ...
  const recordings = await RecordingBySiteHour.findAll({
    attributes: [
      ...
      [sequelize.fn('coalesce', sequelize.fn('sum', sequelize.col('total_duration_in_minutes')), 0), 'recordingMinutes']
    ],
    where: {
      locationProjectId: Number(projectId)
    },
    order: [
  ...
  return Object.fromEntries(recordings.map(r => {
    return [r.get('date'), r.get('recordingMinutes')]
  }))
}
