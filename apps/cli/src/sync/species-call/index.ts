...
export const syncOnlyMissingSpeciesCalls = async (sequelizeArbimon: Sequelize, sequelizeBio: Sequelize, project: Project): Promise<void> => {
  const sql = `
  ...
  ORDER BY ts.id
`
  const speciesIdsArbimon = await sequelizeBio
    .query<{ id: number, id_arbimon: number }>(sql, { type: QueryTypes.SELECT, raw: true })
    .then(allSpecies => allSpecies.map(s => s.id_arbimon))

  const speciesCalls = await getArbimonSpeciesCallsForProjectSpecies(sequelizeArbimon, project.idArbimon, speciesIdsArbimon)
  console.info('| syncOnlyMissingSpeciesCalls =', speciesIdsArbimon.length, speciesCalls.length)
  if (speciesCalls.length === 0) return
  await writeSpeciesCallsToPostgres(sequelizeBio, speciesCalls)
}

export const syncProjectSpeciesCall = async (sequelizeArbimon: Sequelize, sequelizeBio: Sequelize, project: Project): Promise<void> => {
  console.info(`==> START SYNCING: species call ${project.slug} (ID: ${project.idArbimon})`)

  const speciesCalls = await getArbimonSpeciesCallsForProjectSpecies(sequelizeArbimon, project.idArbimon)
