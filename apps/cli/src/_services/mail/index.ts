...
const {
  MAILCHIMP_TRANSACTIONAL_API_KEY: apiKey
} = requireEnv('MAILCHIMP_TRANSACTIONAL_API_KEY')
...
export class MailClient {
   ...
   */
  public async send (to: { email: string, name: string }, template: MailTemplate, content: Record<string, unknown>): Promise<void> {
    const html = generateBody(template, content)
    const subject = generateSubject(template)
    const from = 'no-reply@arbimon.org'
    const message: MessagesMessage = { to: [to], from_email: from, subject, html }
