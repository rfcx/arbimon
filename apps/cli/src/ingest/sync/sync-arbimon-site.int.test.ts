...
describe('ingest > sync > site', () => {
  describe('sync site batch (happy path)', async () => {
    // Arrange
    ...
    await ModelRepository.getInstance(biodiversitySequelize).LocationProject.create(DEFAULT_BIO_PROJECT)

    // Act
    const updatedSyncStatus = await syncArbimonSitesBatch(arbimonSequelize, biodiversitySequelize, syncStatus)

    test('sites are stored in bio db successfully', async () => {
      // Assert
      const sitesInDB = await ModelRepository.getInstance(biodiversitySequelize).LocationSite.findAll({
        where: { idArbimon: { [Op.in]: sites.map(s => s.siteId) } }
      })
      expect(sitesInDB.length).toBe(sites.length)
    })
    ...
    test('project sync log is created', async () => {
      const syncLogInDB = await ModelRepository.getInstance(biodiversitySequelize).SyncLogByProject.findOne({
        where: { locationProjectId: 1, syncDataTypeId: SYNC_CONFIG.syncDataTypeId, syncSourceId: SYNC_CONFIG.syncSourceId },
