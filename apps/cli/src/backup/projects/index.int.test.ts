...
describe('Projects backup', async () => {
  test('generates and zips files', async () => {
    ...
    yauzl.fromBuffer(zipBuffer as Buffer, { lazyEntries: true }, (err, zipfile) => {
      ...
      zipfile.on('end', () => {
        expect(entries).toContain('sites_1.csv')
        expect(entries).toContain('species_1.csv')
        expect(entries).toContain('recordings_1.csv')
        expect(entries).toContain('playlists_1.csv')
        expect(entries).toContain('playlist_recordings_1.csv')
        expect(entries).toContain('templates_1.csv')
        expect(entries).toContain('recording_validations_1.csv')
        expect(entries).toContain('pattern_matching_rois_1.csv')
        expect(entries).toContain('soundscapes_1.csv')
        expect(entries).toContain('rfm_models_1.csv')
        expect(entries).toContain('rfm_classifications_1.csv')
      })
    })
  }, 30_000)
})
