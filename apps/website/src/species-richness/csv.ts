...
export const downloadCsvReports = async (filters: ColoredFilter[], datasets: SpeciesByExportReportRow[][], reportPrefix: string): Promise<void> => {
...
const getCsvFile = async ({ startDate, endDate, sites: siteGroups, otherFilters }: ColoredFilter, dataAsJson: SpeciesByExportReportRow[], reportPrefix: string, exportTime: string, datasetIndex: number): Promise<JsZipFile> => {
