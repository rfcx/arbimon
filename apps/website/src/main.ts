...
import { apiClientArbimonKey, apiClientBioKey, apiClientCoreKey, apiClientGeoKey, apiMediaKey, authClientKey, gtagKey, routeNamesKey, storeKey, togglesKey } from './globals'
...
export const createApp = ViteSSG(appComponent, routerOptions, async ({ app, router, isClient, initialState }) => {
  ...
  if (isClient) {
    ...
    const apiClientGeo = getApiClient(import.meta.env.VITE_GEOCODE_API_BASE_URL, getToken)
    ...
    app
      ...
      .provide(apiClientGeoKey, apiClientGeo)
