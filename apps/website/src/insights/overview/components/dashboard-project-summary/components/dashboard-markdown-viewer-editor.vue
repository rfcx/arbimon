<template>
  <template v-if="status === 'pending'">
    <icon-custom-ic-loading class="block bg-none w-8 h-8 mx-auto mt-6" />
  </template>
  ...
  <template v-else>
    ...
    <template v-else>
      ...
      <MarkdownEditor
        ...
        class="mx-auto"
        :on-save-error-message="props.onSaveErrorMessage"
...
}>(), { characterLimit: DEFAULT_CHARACTER_LIMIT, rawMarkdownText: '', onSaveErrorMessage: '' })
...
const editableMarkdownText = ref(props.rawMarkdownText === '' ? '' : unref(props.rawMarkdownText))
