...
connection.query(`select a.site_id arbimon_site_id, s.external_id stream_id, s.name, s.lat, s.lon, a.date, a.hour, a.species_id, sp.scientific_name, sp.taxon_id, st.taxon, num_of_recordings from
...
where r.site_id in (select site_id from sites where project_id = 1989)
...
limit 5000;`, function (error, results, fields) {
  if (error) throw error;
  const json = JSON.stringify(results)
  writeFile(json, 'raw-species-richness-data.json')
})
