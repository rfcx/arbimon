...
(select r.site_id, date(r.datetime_utc) date, hour(r.datetime_utc) hour, rv.species_id, rv.songtype_id, count(1) num_of_recordings
from recordings r
    left join recording_validations rv on r.recording_id = rv.recording_id and (rv.present = 1 or rv.present_review > 0)
where r.site_id in (select site_id from sites where project_id = 1989) and r.datetime_utc between '2021-04-01' and '2021-04-07'
group by r.site_id, date(r.datetime_utc), hour(r.datetime_utc), rv.species_id, rv.songtype_id) a
